...
class TeamsStreamingCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response:LLMResult, **kwargs:Any) -> Any:
        message_context = "Open AI APIで生成される情報は不正確または不適切な場合がありますが、当社の見解を述べるものではありません。"
        message_blocks = [
...
def handle_mention(event, say):
    ...
    callback = TeamsStreamingCallbackHandler(channel=channel, ts=ts)
...
def lambda_handler(event, context):
    ...
    response = {
        'type': 'message',
        'text': f"hello!!! Mr. {body['from']['name']}. You said : {body['text']}"
    }
    return {
        'statusCode': 200,
    ...
    #AWS Lambda環境のリクエスト情報をappが処理できるように変換してくれるアダプター
    #slack_handler = SlackRequestHandler(app=app)
    #未応答はそのままAWS Lambdaの戻り値として返せます
